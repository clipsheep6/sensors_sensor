...
typedef struct Sensor_SensorEvent {
    int32_t sensorTypeId = -1;  /**< 传感器类型 ID */
    int32_t version = -1;       /**< 传感器算法版本 */
    int64_t timestamp = -1;     /**< 报告传感器数据的时间 */
    int32_t option = -1;       /**< 传感器数据选项，包括测量范围和精度 */
    int32_t mode = -1;          /**< 传感器数据上报方式 (详细参考{@link Sensor_SensorMode}) */
    uint8_t *data = nullptr;         /**< 传感器数据 */
    uint32_t dataLen = 0;      /**< 传感器数据长度 */
} Sensor_SensorEvent;
...
typedef struct Sensor_AccelerometerData {
    float x = 0.0;       /**< 施加在设备x轴的加速度，单位 : m/s² */
    float y = 0.0;       /**< 施加在设备y轴的加速度，单位 : m/s² */
    float z = 0.0;       /**< 施加在设备z轴的加速度，单位 : m/s² */
