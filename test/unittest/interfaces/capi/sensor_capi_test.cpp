...
// constexpr int32_t ACCELEROMETER { 1 };
...
void SensorDataCallbackImpl(Sensor_SensorEvent *event)
{
    ...
    if (event[0].dataLen < sizeof(Sensor_AccelerometerData)) {
        SEN_HILOGE("Event dataLen less than acc data size, event.dataLen:%{public}u", event[0].dataLen);
        return;
    }
    Sensor_AccelerometerData *accelData = reinterpret_cast<Sensor_AccelerometerData *>(event[0].data);
    SEN_HILOGD("ensorId:%{public}d, version:%{public}d, dataLen:%{public}d,"
...
HWTEST_F(SensorCapiTest, OH_Sensor_GetAllSensorsTest_001, TestSize.Level1)
{
    ...
    SEN_HILOGI("sensorInfos.sensorTypeId:%{public}d, count:%{public}d,", sensorInfos->sensorTypeId, count);
    ASSERT_EQ(ret, 0);
    ASSERT_NE(count, 0);
}
...
{
    SEN_HILOGI("OH_Sensor_GetAllSensors_001 in");
    Sensor_SubscribeUser cb;
    cb.callback = SensorDataCallbackImpl;
    Sensor_SubscribeId subscribeId;
    subscribeId.sensorTypeId = ACCELEROMETER;
    Sensor_SubscribeAttribute attribute;
    attribute.samplingInterval = 100000000;
    attribute.reportInterval = 100000000;
    Sensor_Result ret = static_cast<Sensor_Result>(OH_Sensor_SubscribeSensor(subscribeId, attribute, &cb));
    ASSERT_EQ(ret, 0);
...
HWTEST_F(SensorCapiTest, OH_Sensor_UnsubscribeSensorTest_001, TestSize.Level1)
{
    ...
    ASSERT_EQ(ret, 0);
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    ret = static_cast<Sensor_Result>(OH_Sensor_UnsubscribeSensor(subscribeId, &cb));
    ASSERT_EQ(ret, 0);
}
...
HWTEST_F(SensorCapiTest, OH_Sensor_UnsubscribeSensorTest_002, TestSize.Level1)
{
    ...
    Sensor_SubscribeId subscribeId;
    subscribeId.sensorId = INVALID;
    Sensor_Result ret = static_cast<Sensor_Result>(OH_Sensor_UnsubscribeSensor(subscribeId, &cb));
    ASSERT_NE(ret, SENSOR_SUCCESS);
}
